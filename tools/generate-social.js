...
async function generate(vehicleName) {
  const today = new Date().toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric", timeZone: "America/New_York" });
  const sys = `You run the Instagram for SafeWheels Rentals SWFL, a family Turo car-rental business in Southwest Florida (Cape Coral / Fort Myers / Punta Gorda). Cars are rented EXCLUSIVELY via Turo; the company meets guests at PGD and RSW airports and delivers within 50 miles of ZIP 33955. Voice: warm, local, upbeat, like a real local owner — never corporate or AI-sounding. EVERY post's photo features one specific fleet vehicle; weave that exact vehicle into the caption naturally. Each post should COMPLEMENT what's happening in Southwest Florida right now — the current season, a well-known recurring local event/festival/market, a holiday, or a timely seasonal activity — and connect it to the convenience of having that car. IMPORTANT: do NOT invent specific dates, times, or event details you're unsure of; keep references to the season or well-known recurring happenings (e.g. "stone crab season," "snowbird season," summer sunset cruises, holiday boat parades). Never mention a street address.`;
  const res = await fetch("https://api.anthropic.com/v1/messages", {
    ...
    body: JSON.stringify({
      ...
      messages: [{ role: "user", content: `Today is ${today}. Today's photo features our **${vehicleName}**. Write today's Instagram post tied to what's happening in Southwest Florida around now (current season or a well-known recurring event/activity), naturally featuring the ${vehicleName}. Vary the topic from a typical day. Call social_post.` }],
    }),
  });
